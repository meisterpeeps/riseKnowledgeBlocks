  ...
  knowledgeCheckBlocks:  [
    {
      id: 0,
      userState: {
        answerState: {'Cookies and coffee': false, 'Donuts and cider': false},
        isSubmitted: false,
      },
      question: {
    ...
    },
    {
      ...
      question: {
        text: 'What is taco cat an example of?',
        media: {
          type: 'image',
          url: 'assets/tacocat.jpg',
        }
      },
      answers: [
        {
          text: 'Evidence we are in a simulation',
          isCorrect: false
        },
        {
          text: 'A palindrome',
      ...
      feedback: 'taco cat backwards is taco cat'
    }
  ]
}
...
function server() {
  ...
  app.post('/knowledge-check-blocks', (req, res) => {
    const knowledgeBlock = db.knowledgeCheckBlocks.find((block) => block.id === req.body.id);
    knowledgeBlock.userState = req.body.userState;    
    res.sendStatus(200);
  })
  app.get('/', function (req, res) {
    res.sendFile(path.resolve(__dirname, '../client/dist/html', 'index.html'));
  });
